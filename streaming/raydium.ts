...
const client = new Client(
  "https://grpc.solanavibestation.com",
  undefined,
  undefined
      ...
      if (slotCheckResult) {
        ...
        const intervalId = setInterval(async () => {
          ...
          if (Buffer.isBuffer(marketDetails)) {
            const fullMarketDetailsDecoded =
              MARKET_STATE_LAYOUT_V3.decode(marketDetails);
        ...
        filters: [
          {
            memcmp: {
              offset:
                LIQUIDITY_STATE_LAYOUT_V4.offsetOf("quoteMint").toString(), // Filter for only tokens paired with SOL
            ...
            memcmp: {
              offset:
                LIQUIDITY_STATE_LAYOUT_V4.offsetOf(
                  "marketProgramId"
                ).toString(), // Filter for only Raydium markets that contain references to Serum
          ...
          },
          {
            memcmp: {
              offset:
                LIQUIDITY_STATE_LAYOUT_V4.offsetOf(
                  "swapQuoteInAmount"
                ).toString(), // Hack to filter for only new tokens. There is probably a better way to do this
